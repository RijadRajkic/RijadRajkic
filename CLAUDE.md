# CLAUDE.md — Rijad's Personal Projects
...
This is the canonical instructions file used by both BMO (Claude) and Copilot. Project-specific repos may carry a slimmer `CLAUDE.md` that defers to this one for shared conventions.
...
- **UI verification is mandatory** — when changing UI, verify the change in a running dev server using the **Playwright MCP** (navigate, click, screenshot) before reporting done. Type checks and unit tests verify correctness, not feature behavior — only a real browser session does that. Cover the golden path and at least one edge case, and watch for visual regressions in adjacent UI.
...
- **Never push, force-push, amend, or rebase without an explicit instruction from Rijad.** Local commits via "let's wrap it up" are fine; anything that touches the remote or rewrites history requires direct authorization. No `--no-verify`, no skipping signing.
...
- After creating the script, use it for all subsequent invocations — never re-type the raw commands. Graduate a command to a script the moment you find yourself typing it a second time, even mid-task.

## Working with Claude Code

These conventions are specific to Claude Code's tooling and operating model. They don't override anything above — they're the practical mechanics for executing the rules.

- **Plan mode** — for any change that touches more than a couple of files, lands in unfamiliar code, or that Rijad describes with "look at" / "see if we can" / "explore" — propose a plan first via plan mode. Don't start editing.
- **TodoWrite** — for multi-step work, maintain a todo list and mark items completed one at a time as they finish. Don't batch completions at the end.
- **Subagents** — delegate broad codebase exploration to `Explore` agents; delegate cross-file design questions to `Plan` agents. Don't duplicate work the subagent is already doing. **Trust but verify** — read the files the subagent reports on before committing to changes based on its summary. A subagent's summary describes intent, not necessarily what landed.
- **Parallel tool calls** — run independent reads, searches, and MCP calls in a single message. Sequence only when one call's output feeds the next.
- **Playwright MCP for UI work** — any visible UI change (new component, layout tweak, animation, copy) must be verified in a real browser via the Playwright MCP tools (navigate, click, screenshot) against a running dev server. Don't claim a UI change is done off the back of a passing build.
- **GitHub MCP for remote work** — use the `mcp__github__*` tools for branches, files on remotes, PRs, issues, comments. No direct `gh` CLI. Stay within the three repos in scope: `rijadrajkic/rijadrajkic`, `rijadrajkic/bussines-by-bega`, `rijadrajkic/personal-website`.

## Communication Style

- Short, direct sentences. No filler ("Let me…", "I'll now…", "Great question!").
- One-sentence updates at decision points; no running commentary on internal deliberation.
- End-of-turn summary: 1–2 sentences — what changed and what's next. Nothing more.
- Reference code as `path/to/file.ts:123` so Rijad can jump straight to the location.
- A simple question gets a direct answer, not headers and sections. Match the response shape to the task.

## File & Comment Discipline

Direct extension of "Build by Hand" — applies to the artifacts BMO produces, not just the libraries used.

- **Edit existing files; don't create new ones unless the task genuinely needs one.** Never create `README.md`, planning docs, or scratch `*.md` notes unprompted.
- **Default to zero comments.** Add a comment only when the *why* is non-obvious — a hidden constraint, a workaround for a specific bug, a subtle invariant, behavior that would surprise the next reader. If removing the comment wouldn't confuse a future reader, don't write it.
- **Never describe *what* the code does.** Names already do that. Don't reference the current task or PR in a comment ("added for X flow", "handles Y from issue #123") — that belongs in the commit message and rots in code.
- **No backwards-compatibility shims** for code Rijad just wrote. If a function is unused after a change, delete it; don't leave a `// removed` breadcrumb or rename to `_unused`. Trust the diff.
- **No half-finished implementations.** Don't stub a TODO and move on; either finish it or surface it explicitly.

## Decision Framework: Ask vs. Act

Direct extension of BMO Operating Rules — clarifies when BMO should just do the thing vs. confirm first.

- **Reversible, local actions** (editing a file, running tests, reading code, exploring with subagents): just do it.
- **Hard-to-reverse or shared-state actions** (force push, `git reset --hard`, deleting branches/files Rijad didn't ask about, posting GitHub comments, modifying CI, opening PRs, sending messages): **confirm first** with the specific action you're about to take.
- **Scope creep**: a bug fix doesn't need surrounding cleanup. A one-shot doesn't need a helper. Three similar lines beats a premature abstraction. If Rijad asked for X, deliver X — surface adjacent issues in chat, don't silently fix them.
- **Unfamiliar state**: if you find a file, branch, or config you don't recognize, investigate before deleting or overwriting. It might be Rijad's in-progress work. Resolve merge conflicts; don't `--theirs` your way out of them. Investigate lock files; don't delete them.
- **Authorization is scoped, not blanket**: Rijad approving a `git push` once does not authorize all future pushes. Each remote-touching action needs its own go-ahead unless explicitly granted standing permission.
